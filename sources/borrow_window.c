...
GtkWidget *create_borrow_window() {
    ...
    gtk_builder_add_from_file(builder, "/home/god/Projects/Book-Management-System/interface/borrow_window.glade",
                              NULL);
    borrow_window = GTK_WIDGET(gtk_builder_get_object(builder, "user_window"));
    ...
    GtkTreeStore *store = gtk_tree_store_new(N_COLUMNS, G_TYPE_INT, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING,
                                             G_TYPE_STRING,
                                             G_TYPE_STRING, G_TYPE_STRING);//新建树视图
    ...
    g_signal_connect(return_button, "clicked", G_CALLBACK(on_return_button_clicked), NULL);
    return borrow_window;
}

void on_borrow_name_entry_clicked(GtkWidget *entry) {
